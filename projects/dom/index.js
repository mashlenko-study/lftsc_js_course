...
function findAllPSiblings(where) {
  ...
  for (const el of where.children) {
    if (el.tagName === 'P') {
      const prevSibl = el.previousSibling;
      elArray.push(prevSibl);
    }
  }
  return elArray;
}
...
function collectDOMStat(root) {
  const arr = root.querySelectorAll('*');
  const names = new Object();
  const classes = new Object();
  let texts = 0;

  arr.forEach((cur) => {
    if (!(cur.nodeName in names)) {
      names[cur.nodeName] = 1;
    } else {
      names[cur.nodeName] = names[cur.nodeName] + 1;
    }
    if (cur.classList.length > 0) {
      for (let i = 0; i < cur.classList.length; i++) {
        if (!(cur.classList.item(i) in classes)) {
          classes[cur.classList.item(i)] = 1;
        } else {
          classes[cur.classList.item(i)] = classes[cur.classList.item(i)] + 1;
        }
      }
    }
    if (cur.textContent) {
  ...
  return { tags: names, classes: classes, texts: texts };
}
...
function observeChildNodes(where, fn) {
  const elToObserve = where;
  const mutationFunc = (mutations) => {
    mutations.forEach((cur) => {
      if (cur.removedNodes.length) {
        fn({
          type: 'remove',
          nodes: Array.from(cur.removedNodes),
        });
      }
      if (cur.addedNodes.length) {
        fn({
          type: 'insert',
          nodes: Array.from(cur.addedNodes),
        });
      }
    });
  };
  ...
  observer.observe(elToObserve, { subtree: true, childList: true });
}
